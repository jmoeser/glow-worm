...
class TestIncomePageGet:
    def test_renders_form_with_expected_inputs(self, authed_client):
        ...
        assert 'name="monthly_income_amount"' in response.text
...
class TestIncomePagePost:
    def test_creates_new_allocation(self, authed_client, db_session):
        ...
        assert float(alloc.monthly_income_amount) == 5000.0
    ...
    def test_updates_existing_allocation(self, authed_client, db_session):
        ...
        count = db_session.query(IncomeAllocation).count()
        assert count == 1
        db_session.refresh(alloc)
        assert float(alloc.monthly_income_amount) == 6000.0
    ...
    def test_saves_sinking_fund_allocations(
        ...
        junctions = db_session.query(IncomeAllocationToSinkingFund).all()
    ...
    def test_error_when_income_zero(self, authed_client):
        response = authed_client.post(
            "/income",
            data={
        ...
        assert response.status_code == 200
        assert "Income must be greater than zero" in response.text
...
class TestApiGetIncome:
    def test_returns_json_when_allocation_exists(self, authed_client, db_session):
        ...
        assert response.status_code == 200
        data = response.json()
        assert float(data["monthly_income_amount"]) == 5000.0
...
class TestApiPostIncome:
    ...
    def test_updates_existing_returns_200(self, authed_client, db_session):
        ...
        assert response.status_code == 200
        assert float(response.json()["monthly_income_amount"]) == 6000.0
